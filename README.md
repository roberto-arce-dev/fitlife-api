...
Usuarios y entrenadores con perfiles separados
...
- **EntrenadorProfile** (rol: ENTRENADOR)
...
```env
# MongoDB
MONGODB_URI=mongodb://localhost:27017/fitlife_db
...
```env
MONGODB_URI=mongodb+srv://usuario:password@cluster.mongodb.net/fitlife_db
...
**Rol asociado:** `ENTRENADOR`

**Endpoints disponibles:**

#### Obtener mi perfil
```bash
GET /api/entrenador-profile/me
Authorization: Bearer {token}
```
...
#### Listar todos los perfiles (Admin)
```bash
GET /api/entrenador-profile
Authorization: Bearer {token_admin}
```

#### Obtener perfil por userId (Admin)
```bash
GET /api/entrenador-profile/{userId}
Authorization: Bearer {token_admin}
```
...
## 📸 Crear PlanEntrenamientos con Imágenes

### ⚠️ IMPORTANTE: La imagen es OPCIONAL

La imagen **NO es requerida** al crear un planentrenamiento. Puedes:
- ✅ Crear el planentrenamiento SIN imagen
- ✅ Agregar la imagen DESPUÉS usando el endpoint de upload

### 🎯 Flujo Recomendado (Paso a Paso)

#### Paso 1: Crear el PlanEntrenamiento sin imagen

```bash
POST /api/planentrenamiento
Authorization: Bearer {{token}}
Content-Type: application/json

{
  "nombre": "PlanEntrenamiento de ejemplo",
  "descripcion": "Descripción del planentrenamiento",
  "disponible": true
}
```
...
**💡 Nota:** Guarda el `_id` del planentrenamiento creado, lo necesitarás para subir la imagen.

---

#### Paso 2: Subir imagen al PlanEntrenamiento

```bash
POST /api/planentrenamiento/507f1f77bcf86cd799439011/upload-image
Authorization: Bearer {{token}}
...
8. **Copiar** el `_id` del planentrenamiento creado

---

#### Paso 2: Subir Imagen

1. **Ir a:** `Upload por Entidad` → `Upload PlanEntrenamiento Image`
2. **Reemplazar** `{{id}}` en la URL con el ID copiado:
   ```
   http://localhost:3004/api/planentrenamiento/507f1f77bcf86cd799439011/upload-image
   ```
...
**✅ ¡Listo!** Tu planentrenamiento ahora tiene imagen.
...
**Imagen original:**
```
http://localhost:3004/uploads/1700000000000-imagen.jpg
```

**Thumbnail (miniatura):**
```
http://localhost:3004/uploads/thumbnails/thumb-1700000000000-imagen.jpg
```
...
# 2. Crear PlanEntrenamiento
PRODUCTO_ID=$(curl -X POST http://localhost:3004/api/planentrenamiento \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d '{   "nombre": "PlanEntrenamiento de ejemplo",   "descripcion": "Descripción del planentrenamiento",   "disponible": true }' \
  | jq -r '.data._id')

echo "PlanEntrenamiento creado con ID: $PRODUCTO_ID"

# 3. Subir imagen
curl -X POST http://localhost:3004/api/planentrenamiento/$PRODUCTO_ID/upload-image \
  -H "Authorization: Bearer $TOKEN" \
...
- La URL debe empezar con `http://localhost:3004/uploads/`
...
- **PlanNutricional**: `POST /api/plannutricional/{id}/upload-image`

El proceso es idéntico al descrito arriba para PlanEntrenamiento.
...
```bash
curl -X GET http://localhost:3004/api/usuario-profile/me \
...
```bash
curl -X PUT http://localhost:3004/api/usuario-profile/me \
  -H "Authorization: Bearer {access_token}" \
  -H "Content-Type: application/json" \
  -d '{
...
**Archivo:** `fitlife-api.postman_collection.json`
...
│   │
│   ├── entrenador-profile/  # Profile ENTRENADOR
│   │   ├── dto/
│   │   ├── schemas/
│   │   ├── entrenador-profile.controller.ts
│   │   ├── entrenador-profile.service.ts
│   │   └── entrenador-profile.module.ts
│   │
...
├── fitlife-api.postman_collection.json
└── package.json
```
