...
export class EntrenadorProfileService {
  ...
  async findByUserId(userId: string): Promise<EntrenadorProfile | null> {
    return this.entrenadorprofileModel.findOne({ user: new Types.ObjectId(userId) }).populate('user', 'email role').exec();
  }
  ...
  async delete(userId: string): Promise<void> {
    const result = await this.entrenadorprofileModel.deleteOne({ user: new Types.ObjectId(userId) });
